...
export function showAdminDashboard(): void {
    if (!authToken) {
...
async function loadDashboardStats(): Promise<void> {
    try {
        const [cargaisonsResponse, colisResponse] = await Promise.all([
        ...
        const enCours = cargaisons.filter((c: any) => c.etat === 'en_cours').length;
        const livres = cargaisons.filter((c: any) => c.etat === 'livre').length;
...
export function showCargaisonsList(): void {
    const contentDiv = document.getElementById('admin-content-area');
    if (!contentDiv) return;
...
export async function loadCargaisons(): Promise<void> {
    try {
        const response = await fetch(`${API_BASE}/cargaisons`);
...
export function displayCargaisonsTable(cargaisons: CargaisonData[]): void {
                    ...
                    ${cargaisons.map(cargaison => `
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ${cargaison.lieuDepart?.nom || 'N/A'} → ${cargaison.lieuArrivee?.nom || 'N/A'}
                            </td>
...
export function showColisList(): void {
    const contentDiv = document.getElementById('admin-content-area');
    if (!contentDiv) return;
...
async function loadColis(): Promise<void> {
    try {
...
function displayColisTable(colis: ColisData[]): void {
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ${colis.map(c => `
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${c.client?.nom || 'N/A'}</td>
...
export async function searchColis(): Promise<void> {
    ...
    try {
        const response = await fetch(`${API_BASE}/colis?code=${searchTerm}`);
        const colis = await response.json();
...
export function showCreateColis(): void {
...
export async function deleteCargaison(numero: string): Promise<void> {
    if (!confirm('Êtes-vous sûr de vouloir supprimer cette cargaison ?')) {
        return;
    }
    
    try {
        const response = await fetch(`${API_BASE}/cargaisons/${numero}`, {
            method: 'DELETE',
            headers: {
...
export async function deleteColis(code: string): Promise<void> {
    if (!confirm('Êtes-vous sûr de vouloir supprimer ce colis ?')) {
        return;
    }
    
    try {
        const response = await fetch(`${API_BASE}/colis/${code}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
